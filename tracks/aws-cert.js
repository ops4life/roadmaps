...
<li><strong>Tagging strategy</strong>: enforce mandatory cost-allocation tags (e.g., <code>CostCenter</code>, <code>Environment</code>, <code>Owner</code>) via <strong>AWS Config rules / Tag Policies (Organizations)</strong> or SCPs that deny resource creation without required tags — then activate those as <strong>cost allocation tags</strong> in Billing so they appear in Cost Explorer/CUR breakdowns.</li>
        ...
        { id: "org-practice-exam-notes", label: "Practice Exam Cheat Sheet Notes",
          description: "Distilled service-distinction facts and exam traps for organizational-complexity scenarios, gathered from practice-exam review explanations.",
          content: `<h3 id="scp-behavior">6.1 SCP Behavior and Rollout</h3><ul>
<li>A default <strong>FullAWSAccess</strong> SCP is attached to every root, OU, and account and allows everything until you start attaching your own. An explicit <strong>Deny</strong> in any SCP up the hierarchy always wins over an <strong>Allow</strong> anywhere else; anything not explicitly allowed by every SCP level down to the account is implicitly denied.</li>
<li>AWS explicitly recommends against attaching new/modified SCPs directly to the <strong>root</strong> without testing — attach to an OU first (or a temporary "Onboarding" OU with a looser SCP) and migrate accounts in small batches so you don't lock users out org-wide.</li>
<li><strong>SCPs never affect service-linked roles</strong> — those are exempt so AWS services can keep integrating with Organizations regardless of what a Deny SCP blocks for regular principals.</li>
<li>SCPs only ever <em>restrict</em> the ceiling of permissions; the account/user/role still needs an actual IAM <strong>Allow</strong> policy to do anything. Don't confuse SCPs with IAM policies when a question asks which one "controls the list of services an account can use" (SCP) vs. which one grants the actual permission (IAM policy).</li>
</ul><h3 id="cross-account-sharing-patterns">6.2 Cross-Account Resource Sharing Patterns</h3><ul>
<li><strong>Resource-based policy</strong> (attached to the resource itself) lets a user access a resource in another account <em>without giving up their own account's permissions</em> — different from assuming a role, which swaps the user into that role's permission set entirely. Pair with <strong>AWS Config</strong> rules to continuously audit and alert on policy drift.</li>
<li><strong>AWS RAM trusted access with Organizations</strong> (<code>enable-sharing-with-aws-organizations</code>) is the simplest way to let a trusted AWS service operate org-wide — it auto-creates the needed service-linked role for you, so manually crafting a service-linked role's trust/permissions policy is unnecessary and, for a service-linked role, its trust policy can't be edited anyway.</li>
<li>For a <strong>hub-and-spoke shared-services VPC</strong> serving private DNS to every account: put a private hosted zone on the hub VPC, VPC-peer each spoke VPC to it, then programmatically <strong>associate</strong> each spoke VPC to that one hosted zone — pointing each spoke's own Route 53 NS records at the hub does not work for private zones.</li>
</ul><h3 id="org-wide-logging-billing">6.3 Org-Wide Logging, Auditing, and Billing</h3><ul>
<li>A CloudTrail trail only covers <strong>global services</strong> (IAM, STS, CloudFront, Route 53) when the <code>--include-global-service-events</code> flag is set, and only spans every Region when <code>--is-multi-region-trail</code> is also set — both flags, one new trail, one destination S3 bucket with IAM roles + bucket policy + MFA Delete is the standard "durable, tamper-evident, org-wide audit log" answer. <strong>AWS Config</strong> (not CloudWatch) is the service to monitor for unexpected org membership/config changes and pair with EventBridge + SNS for near-real-time alerting.</li>
<li>Consolidated billing shares Reserved Instance/Savings Plan discounts automatically across every member account; the <strong>payer (management) account</strong> can turn this off per member account in Billing Preferences — there's no "private" per-account opt-out and removing the account from the Org is not the minimal fix.</li>
<li>Comparing purchase options for predictable vs. bursty load: <strong>Compute Savings Plans</strong> (flexible across instance family/EC2/Fargate/Lambda) for steady-state compute, <strong>Spot</strong> for scale-out peaks, and a <strong>1-year All Upfront Reserved Instance</strong> for a database with fully predictable usage gives the deepest discount — Partial/No Upfront RIs save less for the same term.</li>
</ul><h3 id="hybrid-identity-network">6.4 Hybrid Identity and Network for the Org</h3><ul>
<li><strong>AWS Direct Connect Gateway</strong> lets one DX connection's <strong>private</strong> virtual interface reach many VPCs/VPGs across accounts and Regions without a separate BGP session per VPC — the answer whenever a question asks for "inter-Region VPC access over Direct Connect without per-VPC connections."</li>
<li><strong>AWS IAM Identity Center supports SSO to web-browser-based business applications only</strong> — a public-facing <strong>mobile app</strong> needing federated login from a custom or social IdP needs a custom SAML/OIDC solution paired with STS (<code>AssumeRoleWithWebIdentity</code>) or Amazon Cognito, not IAM Identity Center.</li>
<li>Keeping cross-account traffic entirely off the public internet rules out IPSec VPN tunnels and third-party VPN appliances (both traverse the internet); use <strong>VPC Peering</strong> plus <strong>VPC Flow Logs → CloudWatch Logs → cross-account subscription filter</strong> to a central security account instead. Direct Connect cannot connect VPC-to-VPC directly — it's for on-prem-to-VPC only.</li>
</ul>` },
      ]},
...
<li><strong>Elasticity</strong>: Auto Scaling (target tracking &gt; step &gt; simple scaling policies, in that preference order for most cases), Aurora Serverless v2 for unpredictable DB load, Lambda/Fargate for workloads with no idle-capacity tolerance.</li>
        ...
        { id: "ns-practice-exam-notes", label: "Practice Exam Cheat Sheet Notes",
          description: "Distilled service-distinction facts and exam traps for new-solution design scenarios, gathered from practice-exam review explanations.",
          content: `<h3 id="conversational-ai-contact-center">6.1 Conversational AI and Contact Center</h3><p><strong>Amazon Connect</strong> (cloud contact center) + <strong>Amazon Lex</strong> (ASR + NLU chatbot) + <strong>Lambda</strong> (to query business systems and return data) is the standard "let callers self-serve without an agent" stack. Don't confuse it with <strong>Amazon Comprehend</strong> (text-insight extraction, no speech recognition) or <strong>Amazon Polly</strong> (text-to-speech only, no intent recognition) — neither can build the conversational bot itself.</p><h3 id="event-driven-cost-reduction">6.2 Event-Driven Triggers and Cost Reduction</h3><ul>
...
<li><strong>DynamoDB Auto Scaling</strong> (built on AWS Application Auto Scaling) adjusts provisioned R/W capacity to a target utilization automatically — cheaper and simpler than manual capacity planning.</li>
</ul><h3 id="streaming-and-ml-services">6.3 Streaming Ingestion and ML Service Selection</h3><ul>
<li><strong>Kinesis Data Streams</strong> is the answer whenever a scenario needs continuous, high-volume ingestion feeding a <strong>real-time</strong> dashboard/analytics pipeline — SQS, S3, EMR, and Redshift alone don't provide that low-latency streaming path.</li>
<li><strong>Amazon Rekognition</strong> (face/image ML) pairs with a <strong>Storage Gateway File Gateway</strong> when an on-prem media archive needs to keep working with local-file semantics while a Lambda function processes the S3-backed objects — a <strong>Tape Gateway</strong> can't support this because its virtual tapes are Glacier-backed and not readable in real time.</li>
</ul><h3 id="key-and-secret-management-in-transit">6.4 Encryption Key and Secret Placement</h3><ul>
...
</ul><h3 id="serverless-ci-cd-and-networking">6.5 Serverless CI/CD, ECS Networking, and Container Registry Access</h3><ul>
...
<li>ECS <strong>awsvpc</strong> network mode gives each task its own ENI, letting you attach security groups <em>per task</em> and use <strong>IAM roles for tasks</strong> — never pass long-lived IAM credentials into a container via environment variables, and remember the <strong>bridge</strong> mode (Docker's built-in virtual network) can't have SGs attached at the task level at all.</li>
<li><strong>Fargate CannotPullContainerError</strong> (500, connection timed out to ECR) means the task's ENI has no internet route: tasks in <strong>public</strong> subnets need Auto-assign public IP <strong>ENABLED</strong>; tasks in <strong>private</strong> subnets need it <strong>DISABLED</strong> plus a <strong>NAT Gateway in a public subnet</strong> referenced by the private subnet's route table (a NAT Gateway placed in a private subnet doesn't help — it needs its own route to an Internet Gateway).</li>
</ul><h3 id="data-retention-and-rollback-safety">6.6 Data Retention and Stack-Deletion Safety</h3><ul>
<li><strong>DynamoDB TTL</strong> deletes expired items automatically at <strong>no extra write-throughput cost</strong> — the default answer for "delete records after N days" over a scheduled Lambda cleanup job or an S3 lifecycle policy (which can't give low-latency reads on the live data first).</li>
...
</ul><h3 id="identity-federation-for-public-apps">6.7 Identity Federation for Public-Facing Apps</h3><ul>
...
<li><strong>CloudFront geo restriction</strong> (whitelist/blacklist by country) is the cheap way to block specific countries from a globally-distributed app — Route 53 geolocation/geoproximity routing policies only control DNS answers, they don't provide CDN-level low-latency delivery or actually block a request at the edge.</li>
</ul>` },
      ]},
...
<li>Tagging (Domain 1 §5.3) enables cost allocation reporting per team/project in CUR/Cost Explorer — a prerequisite most "identify which team is overspending" scenarios assume is already in place, or call out as the first remediation step if missing.</li>
        ...
        { id: "ci-practice-exam-notes", label: "Practice Exam Cheat Sheet Notes",
          description: "Distilled service-distinction facts and exam traps for continuous-improvement scenarios, gathered from practice-exam review explanations.",
          content: `<h3 id="batch-and-asg-troubleshooting">6.1 Batch Processing and ASG Troubleshooting</h3><ul>
...
<li>To troubleshoot instances an Auto Scaling group keeps terminating as unhealthy, <strong>suspend the ASG's "Terminate" process</strong> and use Session Manager to log in — enabling EC2 <em>instance termination protection</em> is not enough, because the ASG can still terminate instances it created regardless of that setting.</li>
<li>An intermittent (e.g., ~50%) outbound-connectivity failure rate across two AZs, with NACLs otherwise fine, is the classic signature of a <strong>failed NAT instance in one AZ</strong> (AWS doesn't auto-recover NAT instances) — replace NAT instances with <strong>NAT Gateways</strong>, which are managed and redundant within each AZ they're deployed in.</li>
</ul><h3 id="waf-shield-placement">6.2 WAF, Shield, and NACL Placement</h3><ul>
...
</ul><h3 id="static-site-and-dns-gotchas">6.3 Static Site and Hybrid DNS Gotchas</h3><ul>
...
<li><strong>Route 53 Resolver</strong> inbound/outbound endpoints plus forwarding rules connect on-prem DNS resolvers to VPC-hosted private zones and vice versa — the standard hybrid-DNS building block, distinct from simply peering VPCs.</li>
</ul><h3 id="observability-and-alerting-details">6.4 Observability, Health Checks, and Alerting Details</h3><ul>
<li>Prefer the <strong>unified CloudWatch agent</strong> over the legacy CloudWatch Logs agent, an SSM Agent workaround, or a custom Traffic-Mirroring-based daemon — it collects logs and metrics with one agent and needs the least custom engineering. Use <strong>CloudWatch Logs Insights</strong> for ad hoc query-based troubleshooting over raw log tailing.</li>
<li>If an ALB target-group health check hits a page that queries the database, it multiplies DB load with every health-check interval across every instance — switch the <strong>target-group health check to a lightweight static page</strong> and add a separate <strong>Route 53 health check</strong> against a real DB-backed page for genuine end-to-end verification. Note ALB target-group health checks only support <strong>HTTP/HTTPS</strong>, never a raw TCP check.</li>
<li>For a repetitive/duplicate read-heavy query pattern, add an <strong>ElastiCache</strong> layer in front of the database — adding more RDS read replicas alone doesn't reduce duplicate/redundant query volume, and forcing an RDS instance reboot via a "recover" CloudWatch alarm action just causes downtime (and can overload the standby if Multi-AZ).</li>
...
</ul><h3 id="queueing-secrets-and-spot-capacity">6.5 Queueing, Secrets, and Spot Capacity for Existing Workloads</h3><ul>
<li>If messages are landing in a <strong>dead-letter queue</strong> before genuinely slow-but-successful processing can finish, raise the SQS <strong>redrive policy's <code>maxReceiveCount</code></strong> — increasing the visibility timeout or the delivery delay doesn't fix messages that already failed and were re-queued too aggressively.</li>
<li><strong>AWS Secrets Manager</strong> (built-in rotation, dedicated secrets lifecycle) beats Systems Manager Parameter Store SecureString whenever automatic rotation is a stated requirement; for ECS, reference the secret's ARN directly in the container definition's <code>secrets</code> block and grant access via the <strong>task execution role</strong>, not embedded credentials.</li>
<li>To absorb a temporary peak on top of steady-state capacity, add a <strong>Spot Fleet with a diversified allocation strategy across multiple AZs/subnets</strong> under its own Auto Scaling — Reserved or On-Demand instances cost more for a short-lived peak, and any option that skips Auto Scaling entirely fails the "handle peak load automatically" requirement.</li>
</ul>` },
      ]},
...
<li>Identify components to <strong>decouple</strong> (see Domain 2 §4.2 loose-coupling patterns) that were tightly coupled on-prem simply because synchronous calls were the only option available in the old environment — post-migration is the natural point to introduce SQS/SNS/EventBridge between them.</li>
        ...
        { id: "mm-practice-exam-notes", label: "Practice Exam Cheat Sheet Notes",
          description: "Distilled service-distinction facts and exam traps for migration and modernization scenarios, gathered from practice-exam review explanations.",
          content: `<h3 id="rehost-vs-replatform-signal">7.1 Rehost vs. Replatform Signal</h3><p>Simply mirroring an on-prem setup into equivalent EC2 instances (<strong>Rehost</strong>) doesn't gain any of the cloud's elasticity/managed-service benefits — when a question's correct answer swaps a self-managed DB for a managed one (e.g., Dockerized app + MySQL → <strong>ECS + RDS via DMS</strong>) without a full application rewrite, that's <strong>Replatform</strong> ("lift, tinker, and shift"), not Rehost or Refactor.</p><h3 id="vm-migration-tooling">7.2 VM and Container Migration Tooling</h3><ul>
<li><strong>AWS Transform MGN</strong> (the current name/successor to AWS Application Migration Service, built on CloudEndure technology) does continuous block-level replication with <strong>test launches</strong> before a final <strong>cutover launch</strong> — the standard low-downtime rehost path for on-prem/other-cloud VMs. It's a needless complication to split root-volume migration from data-volume migration into separate tools; MGN already replicates all attached/supported volumes together.</li>
<li><strong>VM Import/Export</strong> has no ongoing sync — every re-import is a full one-shot operation, making it a poor fit for "keep replicating changes until cutover" requirements compared to MGN.</li>
<li><strong>Amazon ECS Anywhere</strong> lets on-premises compute, EC2, and Fargate coexist in a <strong>single ECS cluster</strong>, making it the best "start containers on-prem, migrate to the cloud later with one click" path — <strong>EKS Anywhere</strong> is scoped to on-prem/VMware-managed infrastructure only and does not integrate with AWS-managed EKS or Fargate, and <strong>Fargate is not available on AWS Outposts</strong> at all (ECS on Outposts needs EC2 capacity).</li>
</ul><h3 id="database-migration-tooling">7.3 Database Migration Tooling</h3><ul>
<li><strong>Heterogeneous</strong> migrations (different source/target engines, e.g., Oracle → Aurora PostgreSQL) need <strong>AWS SCT</strong> to convert schema/stored procedures/code <em>before</em> <strong>AWS DMS</strong> moves the data (DMS handles the data-type conversions during migration); <strong>homogeneous</strong> migrations (same engine) need DMS alone — SCT is unnecessary there. AWS Glue (ETL/analytics) and AWS SAM (serverless framework) are not schema-conversion tools and are the wrong choice whenever a question dresses them up as one.</li>
<li>For a very large (tens-of-TB+), bandwidth-constrained, time-boxed data warehouse migration, the <strong>AWS Data Transfer Terminal</strong> (a physical walk-in facility to upload local storage devices straight into S3) is a newer offline-transfer option alongside the Snow Family — the same decision driver applies: if the available network pipe can't move the data set within the window, go physical/offline rather than attempt it over VPN/the internet.</li>
</ul><h3 id="post-migration-data-architecture">7.4 Post-Migration Data Architecture Choices</h3><ul>
<li><strong>DynamoDB Global Tables</strong> (multi-Region, multi-active, automatic conflict resolution) is the correct choice whenever a scenario explicitly calls for a <strong>NoSQL</strong>, globally-distributed, low-latency read/write database — a hand-rolled Lambda-based cross-region replay doesn't scale to millions of users, and S3-backed backup/restore replication is far too slow to be "real-time."</li>
<li>Reconfirm the identity-federation rule from Domain 2 in a migration context: <strong>AWS IAM Identity Center only supports SSO for web-browser-based business applications</strong> — a migrated or modernized <strong>mobile</strong> app needing federated login still needs a custom OpenID Connect solution with Cognito Identity Pools, or a custom SAML solution using LDAP + STS, never IAM Identity Center directly.</li>
</ul>` },
      ]},
    ],
};
