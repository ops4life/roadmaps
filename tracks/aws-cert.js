...
<li><strong>AWS Network Firewall</strong> is the AWS-managed alternative to a GWLB + self-managed firewall-appliance fleet for the same centralized-egress pattern: a dedicated inspection VPC with a NAT Gateway, attached to the shared Transit Gateway, running a Network Firewall with firewall rule groups/policies — every spoke account's default route points at the Network Firewall endpoint. A Network Firewall deployed per-account instead still needs a Transit Gateway to give it reachability across VPCs; it doesn't replace the hub.</li>
<li>An <strong>AWS Client VPN</strong> endpoint only needs to be created once, in the account/VPC that already has the Site-to-Site VPN — if that VPC is <strong>peered</strong> with other VPCs, remote clients connecting to that single endpoint can already reach the peered VPCs too (subject to the peered VPCs' own route tables), so there's no need for a Client VPN endpoint per account.</li>
</ul>` },
...
<li><strong>IAM Identity Center</strong> integration with a self-managed on-prem AD requires a <strong>two-way (bi-directional) trust</strong> between AWS Managed Microsoft AD and the self-managed AD — Identity Center needs the reverse direction to read user/group metadata for permission-set assignment, so a one-way trust doesn't work with it. An <strong>AD Connector</strong> is the lighter-weight alternative (a directory proxy, no metadata caching), but it can't be combined with third-party IdP federation at the same time — if a scenario needs both corporate-AD SSO <em>and</em> third-party app federation, Identity Center + two-way trust is the answer, not AD Connector.</li>
...
<li>Cross-account access to a <strong>KMS-encrypted S3 bucket</strong> needs three separate grants aligned, or the requester gets Access Denied with no further explanation: (1) the <strong>S3 bucket policy</strong> in the owning account must name the other account/role as principal, (2) the <strong>KMS key policy</strong> in the owning account must grant that same principal <code>kms:Decrypt</code>, and (3) the requester's own <strong>IAM policy</strong> in their account must allow the S3 actions and the KMS action — missing any one of the three, e.g. an IAM role that has read access to the bucket but no matching KMS key policy grant, still fails.</li>
...
</div><p>Comparing purchase options for predictable vs. bursty load: <strong>Compute Savings Plans</strong> (flexible across instance family/EC2/Fargate/Lambda) for steady-state compute, <strong>Spot</strong> for scale-out peaks, and a <strong>1-year All Upfront Reserved Instance</strong> for a database with fully predictable usage gives the deepest discount — Partial/No Upfront RIs save less for the same term.</p><p>Under <strong>consolidated billing</strong>, an EC2 Reserved Instance purchased in one member account benefits <em>any</em> other member account running matching, unused instance capacity — but only if that capacity is in the <strong>same Availability Zone</strong> the RI was purchased in (matching instance type/Region alone is not enough; each account's own AZ names map to different physical locations anyway, so the match is evaluated per the purchasing account's AZ). A member account running the matching instance type in a different AZ gets no RI benefit at all, even under the same Organization.</p><p><strong>DynamoDB reserved capacity</strong> (a one-time upfront fee plus a committed minimum read/write throughput over a term) offers the same kind of savings over on-demand/standard provisioned throughput as EC2 RIs do over On-Demand — worth calling out whenever a DynamoDB cost-reduction question doesn't clearly need a NoSQL-to-something-else migration, since dropping/recreating tables or adding ElastiCache in front usually cost more.</p><h3 id="right-sizing-tagging-cost-allocation">5.3 Right-Sizing, Tagging, and Cost Allocation</h3><ul>
...
<li>The lowest-effort way to hard-block untagged resource creation org-wide (not just detect it) is an <strong>SCP</strong> with a <code>Condition</code> using the <code>ForAllValues</code> qualifier on the <code>aws:TagKeys</code> condition key, attached at the OU level so it automatically cascades to every member account — an equivalent IAM policy applied individually to each account's users does the same restriction but doesn't scale, since it has to be recreated per account. AWS Config rules only audit/alert on tag compliance; they can't deny the API call itself the way an SCP condition can.</li>
</ul>` },
      ]},
...
<tr><td colspan="2"><strong>UDP/gaming workload note</strong>: <strong>AWS WAF cannot protect a Network Load Balancer</strong> at all (WAF only attaches to ALB/API Gateway/CloudFront/AppSync) — to block unwanted traffic patterns (e.g., non-UDP) in front of an NLB, use a <strong>Network ACL deny rule</strong> on the load balancer's subnets instead. Register the NLB's <strong>Elastic IP as a Shield Advanced Protected Resource</strong> for faster DDoS detection/mitigation on that specific IP.</td></tr>
</tbody>
</table>
...
<li>A CloudFront distribution with a <strong>single default origin</strong> (e.g., an ALB) returns 404 for static assets that actually live in a separate, OAC-protected S3 bucket — one distribution can serve <em>both</em> dynamic and static content only if it has <strong>multiple origins</strong> (one per backend) plus a <strong>cache behavior with a path pattern</strong> (e.g. <code>/static/*</code>) routing matching requests to the S3 origin, leaving the default (<code>*</code>) behavior pointed at the ALB. Routing static-asset requests through the ALB instead still fails, since the OAC-secured S3 bucket policy only trusts the CloudFront distribution, not the load balancer.</li>
</ul>` },
...
</div><p><strong>AWS AppSync</strong>: managed GraphQL API service with built-in <strong>WebSocket-based subscriptions</strong> for pushing real-time data updates to millions of clients (e.g., live comments/chat) — the answer whenever a requirement calls for genuinely real-time updates, not just low-latency polling or API Gateway response caching, which both still leave a gap between when data changes and when a client sees it.</p><h3 id="ha-operations">4.3 High-Availability Operations</h3><ul>
...
<li><strong>Amazon RDS Proxy</strong> sits in front of RDS/Aurora and cuts failover-related outage time — up to a <strong>66% reduction</strong> versus a plain failover — by bypassing DNS caching, preserving idle application connections through the failover (only connections mid-transaction/statement are dropped), immediately accepting new connections, and queueing writes until the new writer is ready, instead of the application seeing a hard connection refusal. A plain RDS Multi-AZ failover with no proxy still takes roughly 60 seconds with one standby (about 35 seconds with two readable standbys) before writes resume.</li>
</ul>` },
...
<li>Moving an existing instance into (or between) placement groups only requires <strong>stopping</strong> it first, then changing its placement group and starting it again — there is no need to terminate and relaunch it, and no need to stop/restart every other instance already in the group.</li>
...
</div><p>Decision driver: <strong>available bandwidth and data volume</strong>. Rule of thumb the exam expects — if transferring over the network would take <strong>weeks</strong>, use the Snow Family; if there's a decent pipe and it's an ongoing/scheduled sync, use DataSync. For a very large (tens-of-TB+), bandwidth-constrained, time-boxed data warehouse migration, the <strong>AWS Data Transfer Terminal</strong> (a physical walk-in facility to upload local storage devices straight into S3) is a newer offline-transfer option alongside the Snow Family — same decision driver: if the available network pipe can't move the data set within the window, go physical/offline. Data Transfer Terminal's reservation/facility overhead only pays off at that tens-to-hundreds-of-TB scale; for a smaller dataset (e.g., ~1 TB) that still can't fully transfer within a short cutover window on a limited-bandwidth link, the cheaper answer is the <strong>AWS CLI's <code>aws s3 sync</code></strong> command run days ahead of the cutover to pre-copy the bulk of the data, followed by one short final sync right before cutover — <code>sync</code> only transfers the delta, so the last pass finishes in a fraction of the time a full transfer would take.</p><pre><code>aws datasync create-task --source-location-arn arn:aws:datasync:...:location/loc-0nfs \\
...
<li>Migrating a legacy <strong>IBM</strong> stack (WebSphere + MQ + Db2) follows a mixed-strategy pattern rather than one single tool: <strong>Db2</strong> converts and migrates via <strong>DMS + SCT</strong> to Amazon Aurora (heterogeneous, needs SCT for the dialect conversion); <strong>WebSphere</strong> rehosts onto an Auto Scaling group of EC2 instances behind an ELB; <strong>IBM MQ</strong> re-platforms to <strong>Amazon MQ</strong> in a phased approach (move producers/consumers over gradually, decommission IBM MQ once all are migrated) — Amazon MQ, not SQS (Standard or FIFO), is the correct target, since SQS lacks several IBM MQ protocol/feature capabilities the exam expects you to recognize as a gap.</li>
